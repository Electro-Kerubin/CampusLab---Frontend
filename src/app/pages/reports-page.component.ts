    ...
    <div class="p-6 space-y-6">
      ...
      @if (hasNoReportData()) {
        <div class="text-xs text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
          ⚠️ ms-campuslab-report no tiene datos todavía — se llena consumiendo eventos Kafka del topic
          <code class="font-mono">bookings.events</code>. Si Kafka no está corriendo, o no hubo reservas
          recientes, esta sección queda vacía (no es un error). "Tasa Aprobación", "Tasa Cancelación" y
          "Distribución de Estados" sí muestran datos reales porque se calculan directo desde las reservas cargadas.
        </div>
      }
      ...
      <div class="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
          <h3 class="font-semibold text-gray-900 text-sm mb-4">Reservas por Hora (últimas 24h)</h3>
          @if (data.hourlyMetrics().length === 0) {
            <div class="h-[200px] flex items-center justify-center text-xs text-gray-400">Sin datos en las últimas 24h</div>
          } @else {
        ...
        <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
          <div class="flex items-center justify-between mb-4">
            <h3 class="font-semibold text-gray-900 text-sm">Tiempo de Ciclo (minutos, por hora)</h3>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div class="xl:col-span-2 bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
          ...
          @if (data.topResourcesView().length === 0) {
            <p class="text-xs text-gray-400">Sin datos en los últimos 7 días</p>
          } @else {
            <div class="space-y-3">
              @for (r of data.topResourcesView(); track r.name; let i = $index) {
        ...
        <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
          ...
          @if (data.statusDistribution().length === 0) {
            <p class="text-xs text-gray-400">Sin reservas cargadas todavía</p>
          } @else {
              ...
              @for (e of data.statusDistribution(); track e.name) {
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-gray-600">{{ statusLabel(e.name) }}</span>
                  </div>
...
export class ReportsPageComponent {
  ...
  hasNoReportData = computed(
    () => this.dataService.hourlyMetrics().length === 0 && this.dataService.resourceUsage().length === 0
  );

  kpis = computed(() => {
    const k = this.dataService.reportKpis();
    return [
      { label: 'Reservas (24h, ms-report)', value: `${k.totalReservas}`, icon: Activity, color: 'bg-green-50 text-[#5cb85c]' },
      { label: 'Tasa Aprobación (todas)', value: `${k.tasaAprobacion}%`, icon: TrendingUp, color: 'bg-blue-50 text-blue-600' },
      { label: 'Tiempo de Ciclo Prom. (24h)', value: `${k.tiempoCicloPromedio}h`, icon: Clock, color: 'bg-yellow-50 text-yellow-600' },
      { label: 'Tasa Cancelación (todas)', value: `${k.tasaCancelacion}%`, icon: Activity, color: 'bg-purple-50 text-purple-600' },
    ];
  });

  maxTopUsos = computed(() => this.dataService.topResourcesView()[0]?.usos ?? 1);
  ...
  // Reservas por hora (últimas 24h) — bookingsCount por bucketHour, de ms-report
  reservasChartData = computed<ChartConfiguration['data']>(() => {
    const sorted = [...this.dataService.hourlyMetrics()].sort((a, b) => a.bucketHour.localeCompare(b.bucketHour));
    return {
      labels: sorted.map((d) => new Date(d.bucketHour).toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' })),
      datasets: [
        {
          label: 'Reservas',
          data: sorted.map((d) => d.bookingsCount),
          borderColor: '#5cb85c',
  ...
  // Tiempo de ciclo por hora (minutos) — avgCycleMinutes por bucketHour, de ms-report
  cicloChartData = computed<ChartConfiguration['data']>(() => {
    ...
    return {
      ...
      datasets: [
        {
          label: 'Ciclo (min)',
          data: sorted.map((d) => d.avgCycleMinutes ?? 0),
  ...
  cicloChartOptions: ChartOptions = {
    ...
    scales: {
      x: { ticks: { font: { size: 10 }, color: '#9ca3af' }, grid: { color: '#f0f0f0' } },
      y: {
        ticks: { font: { size: 10 }, color: '#9ca3af', callback: (v) => `${v} min` },
        grid: { color: '#f0f0f0' },
      },
    },
  };

  // Distribución estados (donut) — derivado de bookings(), no de ms-report
  estadosChartData = computed<ChartConfiguration['data']>(() => ({
    labels: this.dataService.statusDistribution().map((e) => this.statusLabel(e.name)),
    datasets: [
      {
        ...
        borderColor: '#fff',
        borderWidth: 2,
        // @ts-ignore - cutout es válido para datasets doughnut en Chart.js 4
        cutout: '55%',
      },
    ],
  }));
